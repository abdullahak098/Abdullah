...
const Graph = () => {
  ...
  useEffect(() => {
    const fetchPrayerRecord = async () => {
      try {
        const savedRecordJSON = await AsyncStorage.getItem('prayerRecord');
        if (savedRecordJSON) {
          const savedRecord = JSON.parse(savedRecordJSON);
          const { checkedPrayers, checkedGroupPrayers } = savedRecord;

          const numPrayersOffered = checkedPrayers.length + checkedGroupPrayers.length;
          setPrayersOffered(numPrayersOffered);

          const updatedData = {
            ...graphData,
            datasets: [
              {
                data: graphData.labels.map(prayer => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Prayer Record Graph</Text>
      <LineChart
        data={graphData}
        width={350}
        height={220}
      ...
      <Text style={styles.prayerOfferedText}>You Offered {prayersOffered} out of 5 Prayers</Text>
